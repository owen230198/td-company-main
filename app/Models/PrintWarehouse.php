...
class PrintWarehouse extends Model
{
    ...
    static function getFieldSearch()
    {
        ...
        $fields[0]['child'][1]['attr'] = '{"required":1, "inject_class":"__select_child"}';
    ...
    {
        $length = !empty($data['length']) ? $data['length'].'x' : 'Lô ';
        return getFieldDataById('name', 'materals', $data['supp_price']).' - '.$data['qtv'].' - '.$length.$data['width'];
    }
