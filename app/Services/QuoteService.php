...
class QuoteService extends BaseService
{
    ...
    public function getListDataQuote($quote_id, $field="*")
    {
        $list_data['listPapers'] = $this->q_papers->select($field)->where('quote_id', $quote_id)->get()->toArray();
        $list_data['listCatons'] = $this->q_cartons->select($field)->where('quote_id', $quote_id)->get()->toArray();
        $list_data['listFoams'] = $this->q_foams->select($field)->where('quote_id', $quote_id)->get()->toArray();
        $list_data['listSilks'] = $this->q_silks->select($field)->where('quote_id', $quote_id)->get()->toArray();
        $list_data['listFinishes'] = $this->q_finishes->select($field)->where('quote_id', $quote_id)->get()->toArray();
        return $list_data;
    }

    public function refreshQuoteTotal($quote_id)
    {
        $list_data = $this->getListDataQuote($quote_id, 'total_cost');
