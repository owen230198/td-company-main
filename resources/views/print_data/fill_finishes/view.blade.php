        ...
        @if (!empty($fill['stage']) && is_array($fill['stage']))
            @foreach ($fill['stage'] as $fill_stage)
                <p class="d-flex align-items-center mb-1 pb-1 border_bot_eb">
                    ...
                    <span class="w_120 d-block">{{ str_replace('CHI PHÍ', '', getFieldDataById('name', 'materals', $fill_stage['materal'])) }}</span> 
                    <span class="ml-1">: {{ getFieldDataById('name', 'devices', $fill_stage['machine']) }} - KT : {{ $fill_stage['length'] .' x '.  $fill_stage['width'] }}</span>
                </p> 
            @endforeach   
        @endif
    </div>
        ...
        <div class="mt-3">
            ...
            @if (!empty($finish['stage']) && is_array($finish['stage']))
                @foreach ($finish['stage'] as $finish_stage)
                    <p class="d-flex align-items-center mb-1 pb-1 border_bot_eb">
                        <i class="fa fa-asterisk mr-1 fs-14 color_yellow" aria-hidden="true"></i>
                        <span class="w_120 d-block">{{ getFieldDataById('name', 'devices', $finish_stage['materal']) }}</span> 
                        <span class="ml-1">: Có </span>
                    </p> 
                @endforeach   
            @endif
            <p class="d-flex align-items-center mb-1 pb-1 border_bot_eb">
                <i class="fa fa-asterisk mr-1 fs-14 color_yellow" aria-hidden="true"></i>
                <span class="w_120 d-block">SL sản phẩm</span> 
                <span class="ml-1">: {{ $data_item->product_qty }} </span>
            </p> 
        </div>
    </div>
@endsection