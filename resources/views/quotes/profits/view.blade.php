			...
			<div class="quote_bg_content">
				<div class="table_quote my-lg-4 my-3">
					@if ($data_quotes['group_product']== \App\Constants\NameConstant::HARDBOX)
						@include('quotes.profits.hardbox_tables')
					@else
						@include('quotes.profits.paper_tables')
					@endif
				    <div class="text-center p-2 border_grey">
				    	<p class="fs-23 color_red font_bold mb-1">TỔNG GIÁ : {{ number_format($data_quotes['total_cost']) }} VNĐ</p>
				    </div>
				</div>
			</div>
		</div>	
	</div>
	<form action="config-profit/{{ $data_quotes['id'] }}" class="formActionS possition-relative" method="POST">
		@csrf
		<div class="form-group mb-3 d-md-flex flex-wrap align-items-center">
			...
			<div class="d-flex align-items-center">
				<input class="form-control fs-15 short_input" type="number" name="ship_price" value="{{ @$data_quotes['ship_price']?(int)$$data_quotes['ship_price']:'' }}" min="0" required>
			</div> 	
		</div>
		<div class="form-group mb-3 d-md-flex flex-wrap align-items-center">
			<label class="base_label mr-2 label_quotes">Nhập % lợi nhuận</label>
			<div class="d-flex align-items-center">
				<input class="form-control fs-15 short_input" type="number" name="profit" value="{{ @$data_quotes['profit']?(int)$$data_quotes['profit']:'' }}" min="0" required>
			</div> 	
		</div>
